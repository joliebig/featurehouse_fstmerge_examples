                                           ...
                                           DateTime defaultItemDate)
        {
            ...
            while ((nodeRead || reader.Read()) && reader.NodeType != XmlNodeType.EndElement)
            {
                ...
                if (date == now)
                {
                    ...
                    {
                        _log.Warn("Error parsing date from_ item {" + subject +
                                  "} from_ feed {" + link + "}: " + fe.Message);
                        continue;
                    }
                }
                ...
                while ((nodeRead || reader.Read()) && reader.NodeType != XmlNodeType.EndElement)
                {
                    ...
                    if (reader.NamespaceURI.Equals(rssNamespaceUri) || reader.NamespaceURI.Equals(String.Empty))
                    {
                        ...
                        else if (localname == atomized_strings[nt_lastbuilddate])
                        {
                            ...
                            {
                                _log.Warn("Error parsing date from_ channel {" + feedTitle +
                                          "} from_ feed {" + (feedLink ?? f.title) + "}: ", fex);
                            }
                            finally
                            {
                                matched = true;
                ...
                while ((nodeRead || reader.Read()) && reader.NodeType != XmlNodeType.EndElement)
                {
                    ...
                    if (reader.NamespaceURI.Equals(rssNamespaceUri) || reader.NamespaceURI.Equals(String.Empty))
                    {
                                 ...
                                 localname == atomized_strings[nt_updated])
                        {
                            ...
                            {
                                _log.Warn(string.Format("Error parsing date from_ channel {{{0}}} from_ feed {{{1}}}: ", feedTitle, (feedLink ?? f.title)), fex);
                            }
                            finally
                            {
                                matched = true;
