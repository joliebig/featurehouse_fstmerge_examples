...
namespace DiffPatchResources
{
 class DiffPatchResource
 {
  ...
  static void Main(string[] args)
  {
   ...
   foreach (string path in args) {
    ...
    if (makeStrings) {
     ...
     Console.WriteLine("Create .strings from_ {0} ... ", fp);
     if (!sfr.Run(testOnly)) {
  ...
  private static void WriteUsage() {
   ...
   Console.WriteLine("\t-d\t cleanup. Removes data entries from_ language .resx file(s),");
   Console.WriteLine("\t\t that are not anymore available in the main .resx inputfile");
   Console.WriteLine("\t-s\t create .strings file from_ the main .resx inputfile");
   Console.WriteLine();
  }
