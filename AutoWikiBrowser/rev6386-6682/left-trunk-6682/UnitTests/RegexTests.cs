...
work = text
            ...
            Assert.AreEqual(@"<ref name=""Shul726"">Shul, p. 726</ref>", WikiRegexes.NamedReferences.Match(@"<ref name=""vietnam.ttu.edu""/><ref name=""Shul726"">Shul, p. 726</ref>").Value, "match is not across consecutive references â first condensed");
            Assert.AreEqual(@"<ref name=""Shul726"">Shul, p. 726</ref>", WikiRegexes.NamedReferences.Match(@"<ref name=""Shul726"">Shul, p. 726</ref><ref name=""Foo"">foo text</ref>").Value, "match is not across consecutive references â first full");
...
foo
        ...
        public void UntemplatedQuotes()
        {
            ...
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" Â« very fast Â» ", "1").Contains(@"Â« very fast Â»"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" âvery fastâ ", "1").Contains(@"âvery fastâ"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fastâ ", " ").Contains(@"â very fastâ"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â¹ very fast âº ", "").Contains(@"â¹ very fast âº"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            ...
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â ", "").Contains(@"â very fast â"));
            Assert.IsFalse(WikiRegexes.UntemplatedQuotes.Replace(@" âvery
fastâ ", "").Contains(@" âvery
fastâ "));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â but not pretty ", "").Contains("but not pretty"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â but not prettyâ ", "").Contains("but not prettyâ"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" â very fast â and â very well â ", "").Contains(" and "));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@"not pretty but â very fast â ", "").Contains("not pretty but "));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" very fast ", "").Contains(@"very fast"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" 'very fast' ", "").Contains(@"'very fast'"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" ''very fast'' ", "").Contains(@"''very fast''"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" ''very fast'' ", "").Contains(@"''very fast''"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@" ,very fast, ", "").Contains(@",very fast,"));
            Assert.IsTrue(WikiRegexes.UntemplatedQuotes.Replace(@"Now itâs a shame as itâs a", "").Contains(@"Now itâs a shame as itâs a"));
        }
        [Test]
...
|something}}"));
            ...
            Assert.IsFalse(WikiRegexes.Dablinks.IsMatch(@"{{Otheruse2|something}}"));
        }
        [Test]
...
| bye}} "));
        ...
        {
            Assert.IsTrue(WikiRegexes.Pstyles.IsMatch(@"<p style=""margin:0px;font-size:100%""><span style=""color:#00ff00"">âª</span> <small>Francophone minorities</small></p>"));
...
"));
        ...
        public void ArticleIssuesTemplatesTests()
        {
            ...
            Assert.IsTrue(WikiRegexes.ArticleIssuesTemplates.IsMatch(@"{{expert| topic name}}"));
...
Bert").Groups[2].Value, "foo bar\r");
        ...
        public void LinkFGAsCatalan()
        {
            ...
            Assert.IsTrue(WikiRegexes.LinkFGAsCatalan.IsMatch(@"foo {{EnllaÃ§ AD|ar}}"));
        }
        [Test]
        public void LinkFGAsArabic()
        {
            Assert.IsTrue(WikiRegexes.LinkFGAsArabic.IsMatch(@"foo {{ÙØµÙØ© ÙÙØ§ÙØ© ÙØ®ØªØ§Ø±Ø©|he}}"));
            Assert.IsTrue(WikiRegexes.LinkFGAsArabic.IsMatch(@"foo {{ÙØµÙØ© ÙÙØ§ÙØ© ÙØ®ØªØ§Ø±Ø©|
he}}"));
            Assert.IsTrue(WikiRegexes.LinkFGAsArabic.IsMatch(@"foo {{ÙØµÙØ© ÙÙØ§ÙØ© ÙØ®ØªØ§Ø±Ø©
|he}}"));
        }
