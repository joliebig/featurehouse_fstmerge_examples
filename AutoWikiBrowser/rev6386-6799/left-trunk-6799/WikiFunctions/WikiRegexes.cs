        ...
        public static void MakeLangSpecificRegexes()
        {
            ...
            switch(Variables.LangCode)
            {
                case "sv":
                    ...
                    break;
                case "ru":
                    orphantemplate = @"Ð¸Ð·Ð¾Ð»Ð¸ÑÐ¾Ð²Ð°Ð½Ð½Ð°Ñ ÑÑÐ°ÑÑÑ";
            ...
            {
                case "ar":
                    WikiRegexes.LinkFGAs = new Regex(@"{{\s*ÙØµÙØ© ÙÙØ§ÙØ© ÙØ®ØªØ§Ø±Ø©\s*\|.*?}}",
                                                     RegexOptions.Compiled | RegexOptions.Singleline);
                    break;
                case "ca":
                    WikiRegexes.LinkFGAs = new Regex(@"{{\s*([Ll]ink FA|[Ee]nllaÃ§ AD)\|.*?}}",
