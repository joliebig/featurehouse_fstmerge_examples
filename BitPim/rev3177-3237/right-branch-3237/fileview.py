                ...
                for item in e:
                    ...
                    for l in dlist: 
                        if len(d):
                            d+=" - "
                        d+=l
                    self.nodes[index]=(item.name, str(item.size), d)
        ...
        if dict.has_key(self.database_key):
            for i in dict[self.database_key]:
                if dict[self.database_key][i].name==file and dict[self.database_key][i].origin==origin:
                    ...
                    if dict[self.database_key][i].mediadata==data or data==None:
                        return
                    else:
                        del dict[self.database_key][i]
                        break
        else:
