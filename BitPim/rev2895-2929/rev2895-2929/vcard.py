...
class  VFile :
	...
	def next(self):
        ...
        for i in items:
            ...
            if not i.startswith("ENCODING=") and not i=="QUOTED-PRINTABLE": 
...
class  VCard :
	...
	_field_LABEL=_field_ignore
	    _field_BDAY=_field_ignore
	    _field_ROLE=_field_ignore
	    _field_CALURI=_field_ignore
	    _field_CALADRURI=_field_ignore
	    _field_FBURL=_field_ignore
	    _field_REV=_field_ignore
	    _field_KEY=_field_ignore
	    _field_SOURCE=_field_ignore
	...
	def _field_TEL(self, field, value, result):
        ...
        if len(types)==0 or types==["PREF"]: iswork=True 
	...
	_field_CATEGORY=_field_CATEGORIES
...
def out_line(name, attributes, value, formatter, join_char=";"):
    ...
    else: attributes=list(attributes[:]) 
...
def out_eu(vals, formatter, field, bpkey):
    ...
    for v in vals:
        ...
        if len(type):
            ...
            type=["TYPE="+type+["",",INTERNET"][field=="EMAIL"]] 
