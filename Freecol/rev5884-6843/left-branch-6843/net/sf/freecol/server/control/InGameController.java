...
public final class InGameController extends Controller {
    ...
    private Element killPlayerElement(ServerPlayer serverPlayer) {
        ...
        ModelMessage m = new ModelMessage(serverPlayer,
                                          ...
                                          "%nation%", Messages.getNationAsString(serverPlayer));
                ...
                public void run() {
                    try {
                        ...
                        case RAISE_TAX:
                            ...
                            monarchActionElement.setAttribute("goods", Messages.getName(goods));
        ...
        for (Settlement settlement : currentPlayer.getSettlements()) {
            ...
            for (Direction direction : Direction.values()) {
            	...
            	for(Unit unit : unitList){
            		...
            		logger.fine(colony.getName() + " found enemy unit to bombard: " + unit
                                    + "(" + unit.getOwner().getNation() + ")");
	...
	public void unloadCargo(Player player, Goods goods) {
		if (!(goods.getLocation() instanceof Unit)){
			throw new IllegalStateException("Goods not aboard an unit!");
		}
