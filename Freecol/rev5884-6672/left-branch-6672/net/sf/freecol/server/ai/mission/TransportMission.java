...
public class TransportMission extends Mission {
    ...
    protected PathNode findNavalTarget(final int maxTurns) {
        if (!getUnit().isOffensiveUnit()) {
            throw new IllegalStateException("A target can only be found for offensive units. "
                                            + "You tried with: " + getUnit());
        }
        if (!getUnit().isNaval()) {
            throw new IllegalStateException("A target can only be found for naval units. "
                                            + "You tried with: " + getUnit());
        }
