  ...
  public void NullEmailAddressOrUrlShouldNotBeIncludedInXml()
  {
   ...
   Assert.IsNull(XmlUtil.SelectNode(mod.ToXml(), "/modification/url"));
        ...
        public void ShouldReturnTheMaximumChangeNumberFromAllModificationsAsLastChangeNumber()
        {
            ...
            Assert.AreEqual("10", Modification.GetLastChangeNumber(modifications), "from_ Modification.GetLastChangeNumber({10})");
            modifications = new Modification[] { mod1, mod2 };
            Assert.AreEqual("20", Modification.GetLastChangeNumber(modifications), "from_ Modification.GetLastChangeNumber({10, 20})");
            modifications = new Modification[] { mod2, mod1 };
            Assert.AreEqual("20", Modification.GetLastChangeNumber(modifications), "from_ Modification.GetLastChangeNumber({20, 10})");
        }
        [Test]
