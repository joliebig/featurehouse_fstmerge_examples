...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Util
{
 ...
 public class SystemIoFileSystemTest
 {
  ...
  public void ShouldSaveUnicodeToFile()
  {
   SystemPath tempFile = tempRoot.Combine("foo.txt");
   Assert.IsFalse(tempFile.Exists());
   new SystemIoFileSystem().Save(tempFile.ToString(), "hi there? håkan! \u0000");
   Assert.IsTrue(tempFile.Exists());
   using (StreamReader reader = File.OpenText(tempFile.ToString()))
   {
    Assert.AreEqual("hi there? håkan! \u0000", reader.ReadToEnd());
   }
  }
        [Test]
        ...
        public void ShouldSaveUnicodeToFileAtomically()
        {
            SystemPath tempFile = tempRoot.Combine("foo.txt");
            Assert.IsFalse(tempFile.Exists());
            new SystemIoFileSystem().AtomicSave(tempFile.ToString(), "hi there? håkan! \u0000");
            Assert.IsTrue(tempFile.Exists());
            using (StreamReader reader = File.OpenText(tempFile.ToString()))
            {
                Assert.AreEqual("hi there? håkan! \u0000", reader.ReadToEnd());
            }
            new SystemIoFileSystem().AtomicSave(tempFile.ToString(), "hi there? håkan! \u0000 sadfasdf");
            Assert.IsTrue(tempFile.Exists());
            using (StreamReader reader = File.OpenText(tempFile.ToString()))
            {
                Assert.AreEqual("hi there? håkan! \u0000 sadfasdf", reader.ReadToEnd());
            }
        }
        [Test]
