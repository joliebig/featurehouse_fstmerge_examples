...
namespace ThoughtWorks.CruiseControl.Core.tasks.Conditions.DateTimeHelpers
{
    ...
    {
        private static readonly Regex valueSplitter = new Regex(@"^(?<from_>\d{1,2}:\d{2})-(?<To>\d{1,2}:\d{2})$");
        ...
        public TimePeriod(string source)
        {
            ...
            _fromTime = Parse(split.Groups["from_"].Value);
        ...
        public override string ToString()
        {
            ...
            TimeSpan from_ = Inverted ? ToTime : FromTime;
            if (to.Days == 0)
                return string.Format("{0:D2}:{1:D2}-{2:D2}:{3:D2}", from_.Hours, from_.Minutes, to.Hours, to.Minutes);
            return string.Format("{0:D2}:{1:D2}-24:00", from_.Hours, from_.Minutes);
        }
    }
}
