...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol
{
 ...
 public class CvsTest : ProcessExecutorTestFixtureBase
 {
  ...
  protected void CreateCvs()
  {
   ...
   from_ = new DateTime(2001, 1, 21, 20, 0, 0);
   to = from_.AddDays(1);
  }
  [TearDown]
  ...
  {
   ExpectToExecuteArguments(string.Format(@"-q rlog -N -b ""-d>{0}"" module", cvs.FormatCommandDate(from_)));
   ExpectToParseAndReturnNoModifications();
   cvs.Module = "module";
   cvs.WorkingDirectory = DefaultWorkingDirectory;
   cvs.GetModifications(IntegrationResult(from_), IntegrationResult(to));
  }
  [Test]
  public void VerifyLogCommandArgsWithCvsRootAndBranch()
  {
   ExpectToExecuteArguments(string.Format(@"-d myCvsRoot -q rlog -N -rbranch ""-d>{0}"" module", cvs.FormatCommandDate(from_)));
   ExpectToParseAndReturnNoModifications();
  ...
  {
   ExpectToExecuteArguments(string.Format(@"-d myCvsRoot -q rlog -N -b ""-d>{0}"" -wexortech -wmonkey module", cvs.FormatCommandDate(from_)));
   ExpectToParseAndReturnNoModifications();
   cvs.CvsRoot = "myCvsRoot";
   cvs.Module = "module";
  ...
  {
   ExpectToExecuteArguments(string.Format(@"-d myCvsRoot -q rlog -N -S -b ""-d>{0}"" module", cvs.FormatCommandDate(from_)));
   ExpectToParseAndReturnNoModifications();
   cvs.CvsRoot = "myCvsRoot";
   cvs.Module = "module";
  ...
  {
   ExpectToExecuteArguments(string.Format(@"-d :pserver:anonymous@cruisecontrol.cvs.sourceforge.net:/cvsroot/cruisecontrol -q rlog -N -b ""-d>{0}"" cruisecontrol", cvs.FormatCommandDate(from_)));
   ...
   Modification[] modifications = cvs.GetModifications(IntegrationResult(from_), IntegrationResult(to));
  ...
  {
   ExpectToExecuteArguments(string.Format(@"-d :local:C:\dev\CVSRoot -q rlog -N -b ""-d>{0}"" fitwebservice", cvs.FormatCommandDate(from_)));
   Modification mod = new Modification();
  ...
  {
   mockHistoryParser.ExpectAndReturn("Parse", modifications, new IsAnything(), from_, to);
  }
