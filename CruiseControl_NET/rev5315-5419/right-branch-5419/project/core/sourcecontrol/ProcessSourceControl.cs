...
namespace ThoughtWorks.CruiseControl.Core.Sourcecontrol
{
 public abstract class ProcessSourceControl : ISourceControl
 {
  ...
  protected Modification[] GetModifications(ProcessInfo info, DateTime from_, DateTime to)
  {
   ProcessResult result = Execute(info);
   return ParseModifications(result, from_, to);
  }
  ...
  protected Modification[] ParseModifications(ProcessResult result, DateTime from_, DateTime to)
  {
   return ParseModifications(new StringReader(result.StandardOutput), from_, to);
  }
  protected Modification[] ParseModifications(TextReader reader, DateTime from_, DateTime to)
  {
   return historyParser.Parse(reader, from_, to);
  }
