...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Triggers
{
 ...
 public class IntervalTriggerTest : IntegrationFixture
 {
  ...
  public void VerifyThatShouldRequestIntegrationAfterTenSeconds()
  {
   ...
   mockDateTime.SetupResult("Now", new DateTime(2004, 1, 1, 1, 0, 5, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   mockDateTime.SetupResult("Now", new DateTime(2004, 1, 1, 1, 0, 9, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   mockDateTime.SetupResult("Now", new DateTime(2004, 1, 1, 1, 0, 14, 0));
  ...
  public void VerifyThatShouldRequestInitialIntegrationAfterTwoSecondsAndSubsequentIntegrationsAfterTenSeconds()
  {
   ...
   trigger.IntervalSeconds = 10;
   trigger.BuildCondition = BuildCondition.IfModificationExists;
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 0, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()"); ;
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 1, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 2, 0));
            Assert.AreEqual(ModificationExistRequest(), trigger.Fire(), "trigger.Fire()");
   trigger.IntegrationCompleted();
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 3, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 11, 0));
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   mockDateTime.SetupResult("Now", new DateTime(2002, 1, 2, 3, 0, 14, 0));
            Assert.AreEqual(ModificationExistRequest(), trigger.Fire(), "trigger.Fire()");
   trigger.IntegrationCompleted();
            Assert.IsNull(trigger.Fire(), "trigger.Fire()");
   VerifyAll();
