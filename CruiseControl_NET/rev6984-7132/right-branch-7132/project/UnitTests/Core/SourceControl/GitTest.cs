...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Sourcecontrol
{
 ...
 public class GitTest : ProcessExecutorTestFixtureBase
 {
  ...
  protected void CreateGit()
  {
   ...
   from_ = new DateTime(2001, 1, 21, 20, 0, 0, DateTimeKind.Local);
   to = from_.AddDays(1);
   GIT_REMOTE_COMMITS = string.Format(GIT_REMOTE_COMMITS, from_.ToUniversalTime().ToString("R"),
...
<git>
  ...
  public void ShouldCloneIfDirectoryDoesntExist()
  {
   ...
   git.GetModifications(IntegrationResult(from_), IntegrationResult(to));
  }
  [Test]
  ...
  public void ShouldNotGetModificationsWhenHashsMatch()
  {
   ...
   Modification[] mods = git.GetModifications(IntegrationResult(from_), IntegrationResult(to));
  ...
  public void ShouldReturnModificationsWhenHashsDifferent()
  {
   ...
   ExpectToExecuteArguments(GIT_REMOTE_COMMITS);
   mockHistoryParser.ExpectAndReturn("Parse", modifications, new IsAnything(), from_, new IsAnything());
   Modification[] result = git.GetModifications(IntegrationResult(from_), IntegrationResult(to));
