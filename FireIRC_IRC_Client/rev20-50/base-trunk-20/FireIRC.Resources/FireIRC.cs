...
namespace OVT.FireIRC.Resources
{
    public class FireIRCCore
    {
        ...
        static Dictionary<string, FireIRCExtend> extentions = new Dictionary<string, FireIRCExtend>();
        ...
        public static Dictionary<string, FireIRCExtend> Extentions
        {
            get { return FireIRCCore.extentions; }
            set { FireIRCCore.extentions = value; }
        }
        ...
        static public ChannelWindow GetChannel(string s, string c)
        {
            ...
            foreach (ChannelWindow i in primaryForm.MdiChildren)
            {
                if (i.Server == s)
                {
        ...
        static public void DoesWindowExist(string s, string c)
        {
            DoesWindowExist(s, c, "Channel");
        }
        static public void DoesWindowExist(string s, string c, string mode)
        {
        ...
        {
            foreach (ChannelWindow i in FireIRCCore.PrimaryForm.MdiChildren)
            {
                if (i.Server == s)
                {
                    i.IsConnected = false;
            ...
            foreach (ChannelWindow i in FireIRCCore.PrimaryForm.MdiChildren)
            {
                if (i.Server == s)
                {
                    try
                    {
                        i.NickChanged(on, nn);
                        i.Sort();
                        MessageInfo p = new MessageInfo(null, String.Format("{0} has changed there nick to {1}", on, nn));
        ...
        static public void UserQuit(string s, string u)
        {
            foreach (ChannelWindow i in FireIRCCore.PrimaryForm.MdiChildren)
            {
                if (i.Server == s)
                {
                    try
                    {
                        i.UserQuit(u);
                        i.Sort();
                        MessageInfo p = new MessageInfo(null, String.Format("{0} has quit IRC (null)", u));
                        i.Write(Formatter.Format(Formatter.MessageType.IrcCommand, p, Clients[s].IrcClient.Address,null));
                    }
                    catch (KeyNotFoundException) { }
                    catch (NullReferenceException) { }
        ...
        static public void ExecuteLine(string server, string line)
        {
            ...
            {
                HelpEngine.ConsoleHelpEngine.ShowHelp(parms);
            }
            else if (parms[0] == "ns")
            {
