...
namespace OVT.FireIRC.Resources
{
    public class FireIRCCore
    {
        ...
        static public void ExecuteLine(string server, string line)
        {
            ...
            else if (parms[0] == "autoident")
            {
                Clients[server].IrcClient.SendMessage(SendType.Message, "NickServ", "identify " + Clients[server].Ninfo.NickServPass);
            }
            ...
            else if (parms[0] == "server")
            {
                NickInformation nickInformation = FireIRCCore.Settings.ReturnNickInformation("Default Identity");
                Perform m = new Perform();
                m.Perf = new string[] { };
                FireIRCCore.Clients.Add(parms[1], new Client(parms[1], 6667, nickInformation.Nicks, nickInformation.UserName, nickInformation.RealName, m, nickInformation));
            }
            else
            {
