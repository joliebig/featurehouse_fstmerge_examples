...
namespace PluginTest
{
    public class GenericNickStuff : OnLaunch
    {
        public void Run()
        {
            ...
            ToolStripMenuItem ignore = new ToolStripMenuItem(OVT.FireIRC.Resources.Resources.ResourceManagement.r.GetString("FireIRC.NickList.Ignore"));
            ignore.Click += new EventHandler(ignore_Click);
            ToolStripMenuItem unignore = new ToolStripMenuItem(OVT.FireIRC.Resources.Resources.ResourceManagement.r.GetString("FireIRC.NickList.UnIgnore"));
            unignore.Click += new EventHandler(unignore_Click);
           FireIRCCore.ExtEngine.NickList.Items.Add(banusers);
            ...
            FireIRCCore.ExtEngine.NickList.Items.Add("-");
            FireIRCCore.ExtEngine.NickList.Items.Add(ignore);
            FireIRCCore.ExtEngine.NickList.Items.Add(unignore);
        }
        void unignore_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem i in FireIRCCore.GetActiveChannelWindow().SelectedNicks)
            {
                FireIRCCore.Settings.IgnoreList.Remove(FireIRCCore.Clients[FireIRCCore.ActiveServer].IrcClient.GetIrcUser(i.Text).Host);
            }
        }
        void ignore_Click(object sender, EventArgs e)
        {
            foreach (ListViewItem i in FireIRCCore.GetActiveChannelWindow().SelectedNicks)
            {
                FireIRCCore.Settings.IgnoreList.Add(FireIRCCore.Clients[FireIRCCore.ActiveServer].IrcClient.GetIrcUser(i.Text).Host);
            }
        }
