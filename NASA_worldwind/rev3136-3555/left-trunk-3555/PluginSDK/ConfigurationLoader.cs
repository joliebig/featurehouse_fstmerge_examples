...
namespace WorldWind
{
 public class ConfigurationLoader
 {
  ...
  public static World Load(string filename, Cache cache)
  {
            ...
            {
                Log.Write(Log.Levels.Debug, "CONF", "validating " + filename + " against WorldXmlDescriptor.xsd");
            ...
            try
            {
                ...
                if (worldIter.Count > 0)
                {
                    ...
                    World newWorld = new World(
                        ...
                        (terrainAccessor != null ? terrainAccessor[0] : null)
                        );
   ...
   while(iter.MoveNext())
   {
    ...
    if(description != null && description.Length > 0)
     qts.Description = description;
                qts.ImageStores[0].AlphaKeyEnabled = false;
                qts.ImageStores[0].ColorKeyEnabled = false;
    if(iter.Current.Select("TransparentColor").Count > 0)
    {
     System.Drawing.Color c = getColor(iter.Current.Select("TransparentColor"));
     qts.ImageStores[0].ColorKey = c.ToArgb();
                    qts.ImageStores[0].ColorKeyEnabled = true;
    }
    if((iter.Current.Select("TransparentMinValue").Count > 0) && (iter.Current.Select("TransparentMaxValue").Count > 0))
    {
                    qts.ImageStores[0].AlphaKeyMin = int.Parse(getInnerTextFromFirstChild(iter.Current.Select("TransparentMinValue")));
                    qts.ImageStores[0].AlphaKeyMax = int.Parse(getInnerTextFromFirstChild(iter.Current.Select("TransparentMaxValue")));
                    qts.ImageStores[0].AlphaKeyEnabled = true;
                }
   ...
   if(iter.Count > 0)
   {
    while(iter.MoveNext())
    {
     ...
     parentRenderable.RenderPriority = RenderPriority.Placenames;
        ...
        private static void addTiledWFSPlacenameSet(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable)
        {
            if (iter.Count > 0)
            {
                while (iter.MoveNext())
  ...
  private static void addIcon(XPathNodeIterator iter, World parentWorld, RenderableObjectList parentRenderable, Cache cache)
  {
   if(iter.Count > 0)
   {
    while(iter.MoveNext())
    {
     ...
     if (textureFilePath.Length > 0 && !Path.IsPathRooted(textureFilePath) && !textureFilePath.ToLower().StartsWith("http://"))
    ...
    while(iter.MoveNext())
    {
     ...
     if(maximumDisplayAltitudeString != null && maximumDisplayAltitudeString.Length > 0)
      lf.MaximumDisplayAltitude = ParseDouble(maximumDisplayAltitudeString);
                    ...
                    {
                        if(altitudemodeString.ToLower().Equals("absolute")) lf.AltitudeMode = AltitudeMode.Absolute;
                        if(altitudemodeString.ToLower().Equals("relativetoground")) lf.AltitudeMode = AltitudeMode.RelativeToGround;
                        if(altitudemodeString.ToLower().Equals("clampedtoground")) lf.AltitudeMode = AltitudeMode.ClampedToGround;
                    }
