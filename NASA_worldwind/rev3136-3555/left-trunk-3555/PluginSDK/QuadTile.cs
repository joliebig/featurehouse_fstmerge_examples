...
namespace WorldWind.Renderable
{
 public class QuadTile : IDisposable
 {
  ...
  protected TextureSource[] textures;
  ...
  public virtual void Initialize()
  {
   ...
   try
   {
                if (textures == null)
                {
                    textures = new TextureSource[QuadTileSet.ImageStores.Length];
                    for (int i = 0; i < textures.Length; i++)
                    {
                        textures[i] = new TextureSource(QuadTileSet.ImageStores[i].GetDownloadUrl(this),
                            QuadTileSet.ImageStores[i].GetLocalPath(this),
                            QuadTileSet.ImageStores[i],
                            new PriorityCallback(PriorityCallback));
                        textures[i].BasePriority = -this.Level;
                        textures[i].Description = QuadTileSet.Name + " Level " + this.Level + " Tile " + this.Row + "/" + this.Col;
                    }
                }
   ...
   finally
   {
    isInitialized = true;
        ...
        private float PriorityCallback()
        {
            Angle sphd = MathEngine.SphericalDistance(CenterLatitude, CenterLongitude,
                                            DrawArgs.Camera.Latitude, DrawArgs.Camera.Longitude);
            float priority = 100.0f - (float)(200.0*sphd.Radians/Math.PI);
            return priority;
        }
 ...
 && DrawArgs.Camera.ViewFrustum.Intersects(BoundingBox)
  ...
  void Render(Device device, CustomVertex.PositionNormalTextured[] verts, QuadTile child)
  {
   ...
   {
                if (QuadTileSet.EffectEnabled && QuadTileSet.Effect != null)
                {
                    ...
                    try
                    {
                        for (int i = 0; i < textures.Length; i++)
                        {
                            ...
                            effect.SetValue(name, textures[i]);
                        }
