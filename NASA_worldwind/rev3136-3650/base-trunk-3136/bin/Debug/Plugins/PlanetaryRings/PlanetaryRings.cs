...
namespace Murris.Plugins
{
 ...
 public class PlanetaryRingsLayer : RenderableObject
 {
  ...
  bool showGlow = false;
  public PlanetaryRingsLayer(string LayerName, string pluginPath, WorldWindow worldWindow) : base(LayerName)
  {
  ...
  public void ReadSettings()
  {
   ...
   if(line != "")
   {
    ...
    if(settingsList.Length >= 7) sunLat = double.Parse(settingsList[6]);
    if(settingsList.Length >= 8) sunLon = double.Parse(settingsList[7]);
  ...
  public override void Render(DrawArgs drawArgs)
  {
   ...
   if(!sunLocked) sunLat = MathEngine.RadiansToDegrees(sunInvert ? -camLat : camLat);
