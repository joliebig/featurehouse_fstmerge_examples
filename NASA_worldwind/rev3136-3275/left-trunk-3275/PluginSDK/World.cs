  ...
  static int m_nChannels = 4;
     ...
     float m_fRayleighScaleDepth = 0;
            ...
            try
            {
                if (m_meshList.Count > 0 && ((!World.Settings.ForceCpuAtmosphere && m_canDoShaders) || m_opticalDepthBuffer1 != null))
                {
                    ...
                    if (!World.Settings.ForceCpuAtmosphere && m_canDoShaders)
                    {
                        ...
                        int tci1 = drawArgs.device.GetTextureStageStateInt32(1, TextureStageStates.TextureCoordinateIndex);
                        ...
                        drawArgs.device.SetTextureStageState(1, TextureStageStates.TextureCoordinateIndex, tci1);
                    }
                    else
                    {
                        for (int i = 0; i < m_meshList.Count; i++)
        ...
        void device_DeviceReset(object sender, EventArgs e)
        {
            ...
            string outerrors = "";
            skyFromSpaceEffect = Effect.FromFile(
                device,
                "shaders\\SkyFromSpace.fx",
                null,
                null,
                ShaderFlags.None,
                null,
            ...
            skyFromAtmosphere = Effect.FromFile(
                device,
                "shaders\\SkyFromAtmosphere.fx",
                null,
                null,
                ShaderFlags.None,
                null,
