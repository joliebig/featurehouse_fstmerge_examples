...
namespace WorldWind.Widgets
{
 ...
 public class Form : IWidget, IInteractive
 {
  ...
  public virtual void Render(DrawArgs drawArgs)
  {
   ...
   if(!m_AutoHideHeader || (DrawArgs.LastMousePosition.X >= m_Location.X &&
    DrawArgs.LastMousePosition.X <= m_Location.X + m_Size.Width &&
    DrawArgs.LastMousePosition.Y >= m_Location.Y &&
    DrawArgs.LastMousePosition.Y <= m_Location.Y + m_Size.Height))
   {
    ...
    m_TextFont.DrawText(
     null,
     m_Text,
     new System.Drawing.Rectangle(m_Location.X + 2, m_Location.Y, m_Size.Width, m_HeaderHeight),
     DrawTextFormat.None,
     m_TextColor.ToArgb());
    m_WorldWindDingsFont.DrawText(
     null,
     "E",
     new System.Drawing.Rectangle(m_Location.X + m_Size.Width - 15, m_Location.Y + 2, m_Size.Width, m_Size.Height),
  ...
  public bool OnMouseMove(System.Windows.Forms.MouseEventArgs e)
  {
   ...
   else if(m_IsDragging)
   {
    ...
    if(m_Location.Y < 0)
     m_Location.Y = 0;
    if(m_Location.Y + m_Size.Height > DrawArgs.ParentControl.Height)
     m_Location.Y = DrawArgs.ParentControl.Height - m_Size.Height;
    if(m_Location.X + m_Size.Width > DrawArgs.ParentControl.Width)
     m_Location.X = DrawArgs.ParentControl.Width - m_Size.Width;
    handled = true;
   }
  ...
  private bool isPointInCloseBox(System.Drawing.Point absolutePoint)
  {
   ...
   if(absolutePoint.X >= m_Location.X + closeBoxXOffset &&
    absolutePoint.X <= m_Location.X + closeBoxXOffset + closeBoxSize &&
    absolutePoint.Y >= m_Location.Y + closeBoxYOffset &&
