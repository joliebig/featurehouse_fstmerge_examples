  ...
  private void m_RefreshTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
  {
            try
            {
                ...
                if (m_ImageUri.ToLower().StartsWith("http://"))
                {
                    ...
                    if (!saveFile.Exists || (m_RefreshTime > 0 && saveFile.LastWriteTime.Subtract(System.DateTime.Now) > TimeSpan.FromSeconds(m_RefreshTime)))
                    {
                        try
                        {
                ...
                System.IO.FileInfo fi = new System.IO.FileInfo(m_SaveFilePath);
                Utility.Log.Write("loading: " + fi.FullName);
                if (fi.Exists)
                    Utility.Log.Write("EXISTS");
                else
                    Utility.Log.Write("NO EXISTS");
