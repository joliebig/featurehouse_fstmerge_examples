   ...
   System.Collections.ArrayList sortedList = new System.Collections.ArrayList();
        ...
        void ChangeForm(string country, int category)
        {
            ...
            if (value is double)
            {
                ...
                System.Collections.ArrayList sortedDataList = (System.Collections.ArrayList)SortedFieldData[Headers[category]];
                int index = -1;
                double minValue = -1;
                double maxValue = -1;
                for (int i = 0; i < sortedDataList.Count; i++)
                {
     KeyDataPair keyDataPair = (KeyDataPair)sortedDataList[i];
                    if (keyDataPair.Key == country)
                    {
                        index = i;
                    }
                    if (minValue == -1 || keyDataPair.Data < minValue)
                        minValue = keyDataPair.Data;
                    if (maxValue == -1 || keyDataPair.Data > maxValue)
                        maxValue = keyDataPair.Data;
                }
                for(int i = 0; i < m_wavingFlagsList.ChildObjects.Count; i++)
                {
                    if (m_wavingFlagsList.ChildObjects[i] is WavingFlagLayer)
                    {
                        ...
                        {
       KeyDataPair keyDataPair = (KeyDataPair)sortedDataList[j];
                            if (keyDataPair.Key == wavingFlag.Name)
                            {
                                wavingFlag.Bar3D.ScalarValue = keyDataPair.Data;
                                foundScalar = true;
                                break;
                            }
                        }
     ...
     KeyDataPair keyDataPair = (KeyDataPair)sortedDataList[startIndex];
        ...
        private void InitializeCiaForm()
        {
            ...
            m_countryNameLabel.ForeColor = System.Drawing.Color.FromArgb(37, 64, 71);
            ...
            m_currentCategoryLabel.ForeColor = System.Drawing.Color.FromArgb(37, 64, 71);
            ...
            m_currentBodyText.Text = "";
            m_currentBodyText.ForeColor = System.Drawing.Color.FromArgb(37, 64, 71);
