 ...
 public class KMLImporter : WorldWind.PluginEngine.Plugin
 {
  ...
  private void LoadKMLFile()
  {
   ...
   try
   {
    ...
    {
     Utility.Log.Write("KMLImporter: " + ex.ToString());
     MessageBox.Show(
   ...
   foreach (XmlNode node in placemarks)
   {
    try
    {
     ...
     if (xnode != null)
     {
      ...
      desc = RemoveTags(descRaw.Replace("<BR>", "\n").Replace("<br>", "\n").Replace("<br/>", "\n"));
      if (desc.Length > 505)
  ...
  private void ParseScreenOverlays(XmlNode inNode, RIcons layer)
  {
   ...
   if(screenOverlays!=null)
   {
    foreach(XmlNode screenOverlayNode in screenOverlays)
    {
     ...
     ScreenOverlay scoverlay = new ScreenOverlay(name,0,0,uri);
     layer.Add(scoverlay);
    }
   }
  }
   ...
   try
   {
    ...
    if (ver != version)
    {
     ...
     try
     {
      ...
      {
       Utility.Log.Write(String.Format(icy, "{0}: The file signature is not valid!", plugName));
       return;
      }
     }
     catch (Exception ex)
     {
      Utility.Log.Write("Signature checking error:\n" + ex);
      return;
     }
  ...
  private void timer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
  {
   ...
   try
   {
    if (!m_firedStartup || (layer != null && layer.IsOn))
    {
     ...
     string saveFile = Path.GetFileName(url);
     if(saveFile == null || saveFile.Length == 0)
      saveFile = "temp.kml";
     saveFile = Path.Combine(owner.PluginDirectory,saveFile);
 ...
 class RIcons : RenderableObjectList
 {
  ...
  public override bool PerformSelectionAction(DrawArgs drawArgs)
  {
   foreach(RenderableObject ro in m_children)
   {
    ...
    try
    {
     Process.Start(icon.ClickableActionURL);
     return true;
    }
    catch
    {
