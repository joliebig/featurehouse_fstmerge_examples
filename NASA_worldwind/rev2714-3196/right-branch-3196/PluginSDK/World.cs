...
namespace WorldWind
{
 ...
 public class AtmosphericScatteringSphere
 {
  ...
  protected int m_numberSections;
        ...
        System.Collections.ArrayList m_meshList = new System.Collections.ArrayList();
  ...
  static int m_nChannels = 4;
     ...
     float m_fRayleighScaleDepth = 0;
            ...
            try
            {
                if (m_meshList.Count > 0 && ((!World.Settings.ForceCpuAtmosphere && m_canDoShaders) || m_opticalDepthBuffer1 != null))
                {
                    ...
                    if (!World.Settings.ForceCpuAtmosphere && m_canDoShaders)
                    {
                        ...
                        {
       MeshSubset currentMeshSubset = (MeshSubset)m_meshList[i];
                            if (!frustum.Intersects(currentMeshSubset.BoundingBox))
                                continue;
                            ...
                            for (int j = 0; j < numPasses; j++)
                            {
                                shader.BeginPass(j);
                                if (doHighResolution)
                                    drawArgs.device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, currentMeshSubset.HigherResolutionVertices.Length, m_indicesHighResolution.Length / 3, m_indicesHighResolution, true, currentMeshSubset.HigherResolutionVertices);
                                else
                                    drawArgs.device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, currentMeshSubset.Vertices.Length, m_indices.Length / 3, m_indices, true, currentMeshSubset.Vertices);
                                shader.EndPass();
                            }
                            shader.End();
                        }
       ...
       if (!frustum.Intersects(currentMeshSubset.BoundingBox))
                                continue;
                            UpdateColor(drawArgs, currentMeshSubset, doHighResolution);
                            if (doHighResolution)
                                drawArgs.device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, currentMeshSubset.HigherResolutionVertices.Length, m_indicesHighResolution.Length / 3, m_indicesHighResolution, true, currentMeshSubset.HigherResolutionVertices);
                            else
                                drawArgs.device.DrawIndexedUserPrimitives(PrimitiveType.TriangleList, 0, currentMeshSubset.Vertices.Length, m_indices.Length / 3, m_indices, true, currentMeshSubset.Vertices);
