 ...
 public class PluginCompiler
 {
   ...
   foreach(PluginInfo pi in m_plugins)
   {
    if(pi.IsLoadedAtStartup)
    {
     ...
     catch(Exception caught)
     {
      ...
      Utility.Log.Write(LogCategory, message);
  ...
  public void Dispose()
  {
   foreach(PluginInfo pi in m_plugins)
   {
    ...
    {
     Utility.Log.Write("PLUG", "Plugin unload failed: " + caught.Message);
    }
   }
  }
  Assembly Compile( PluginInfo pi, CodeDomProvider cdp )
  {
   ...
   CompilerResults cr = compiler.CompileAssemblyFromFile( cp, pi.FullPath );
  ...
  void AddCompilerReference( string pluginDirectory, string assemblyName )
  {
   try
   {
    if(assemblyName.Length<=0)
     return;
    Assembly referencedAssembly = Assembly.LoadWithPartialName(assemblyName);
