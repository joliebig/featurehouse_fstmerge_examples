...
namespace LayerSet
{
 public class Type_LongitudeCoordinate : Altova.Xml.Node
 {
  public Type_LongitudeCoordinate() : base() { SetCollectionParents(); }
  public Type_LongitudeCoordinate(XmlDocument doc) : base(doc) { SetCollectionParents(); }
  public Type_LongitudeCoordinate(XmlNode node) : base(node) { SetCollectionParents(); }
  public Type_LongitudeCoordinate(Altova.Xml.Node node) : base(node) { SetCollectionParents(); }
  ...
  public ValueType2 GetValue2ValueAtCursor( XmlNode curNode )
  {
   if( curNode == null )
    ...
    return new ValueType2( curNode.InnerText );
  }
  ...
  public void ReplaceValue2At(ValueType2 newValue, int index)
  {
   ReplaceDomChildAt(NodeType.Element, "", "Value", index, newValue.ToString());
        ...
        {
            Type_LongitudeCoordinate parent;
            public Type_LongitudeCoordinate Parent
   {
    set
    {
     parent = value;
   ...
   int nIndex;
   Type_LongitudeCoordinate parent;
   public Value2Enumerator(Type_LongitudeCoordinate par)
   {
    parent = par;
    nIndex = -1;
   }
